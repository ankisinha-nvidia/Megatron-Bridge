...
def _render_tool_call_segment(tool_call: dict) -> str:
    """Render a tool call the same way Qwen VL chat templates render assistant calls."""
    ...
    if isinstance(arguments, dict):
        for args_name, args_value in arguments.items():
            if isinstance(args_value, str):
                rendered_value = args_value
            else:
                rendered_value = json.dumps(args_value, ensure_ascii=False)
            rendered.append(f"<parameter={args_name}>\n{rendered_value}\n</parameter>\n")
    rendered.append("</function>\n</tool_call>")
    return "".join(rendered)


def _gather_assistant_text_segments(example: dict) -> list[str]:
    """Extract assistant-generated spans from the structured conversation example.

    The example schema is expected to be {"conversation": [{"role": ..., "content": [...]} ...]} where
    content is a list of items like {"type": "text"|"image"|..., "text": "..."}. Assistant tool_calls are also
    generated by the model, so they are rendered into the Qwen tool-call XML format and included in the loss span.
    """
    texts: list[str] = []
    for turn in example.get("conversation", []):
        ...
        for tool_call in turn.get("tool_calls") or []:
            rendered_tool_call = _render_tool_call_segment(tool_call)
